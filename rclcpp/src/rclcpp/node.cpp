  ...
  should_add_to_graph_listener_(true), graph_users_count_(0)
{
  ...
  if (pos != std::string::npos) {
    name_ = full_name.substr(0, pos);
    full_name.erase(0, pos + delimiter.length());
    ros_domain_id = (char*)malloc(full_name.length()*sizeof(char));
    strncpy(ros_domain_id, full_name.c_str(), full_name.length());
  } else {
    ros_domain_id = getenv(env_var);
  }
  fprintf(stderr, "[rclcpp::info] Node name: %s, domain: %s\n", name_.c_str(), ros_domain_id);
#else
